...
class Ambassador {
    ...
    public function register_post_type() {
        $labels = array(
            'name'               => __( 'Ambassador', \PS::$theme_name ),
    ...
    public function count_new_post() {
        global $menu, $wpdb;
        $count = $wpdb->get_var( "SELECT COUNT(ID) FROM {$wpdb->posts} WHERE post_type = 'ambassador' and post_status = 'publish'" );
        foreach ($menu as $key => $item) {
            if ($item[2] == 'edit.php?post_type=ambassador') {
