...
use FlexibleShippingDhlExpressProVendor\Monolog\Utils;
use FlexibleShippingDhlExpressProVendor\PhpConsole\Connector;
use FlexibleShippingDhlExpressProVendor\PhpConsole\Handler;
use FlexibleShippingDhlExpressProVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FlexibleShippingDhlExpressProVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \FlexibleShippingDhlExpressProVendor\PhpConsole\Connector $connector = null, $level = \FlexibleShippingDhlExpressProVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('FlexibleShippingDhlExpressProVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\FlexibleShippingDhlExpressProVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FlexibleShippingDhlExpressProVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FlexibleShippingDhlExpressProVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FlexibleShippingDhlExpressProVendor\PhpConsole\Helper::isRegistered()) {
            \FlexibleShippingDhlExpressProVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FlexibleShippingDhlExpressProVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \FlexibleShippingDhlExpressProVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FlexibleShippingDhlExpressProVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FlexibleShippingDhlExpressProVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
