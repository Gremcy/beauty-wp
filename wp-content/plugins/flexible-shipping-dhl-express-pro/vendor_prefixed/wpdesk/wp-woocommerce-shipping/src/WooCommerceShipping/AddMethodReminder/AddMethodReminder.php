...
namespace FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\AddMethodReminder;
...
use FlexibleShippingDhlExpressProVendor\WPDesk\ShowDecision\GetStrategy;
/**
 * Can display and track missing shipping method reminder.
 */
class AddMethodReminder implements \FlexibleShippingDhlExpressProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function hooks()
    {
        ...
        (new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\AddMethodReminder\ClickNoticeTracker($this->method_id))->hooks();
        (new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\AddMethodReminder\DeactivationTrackerData($this->method_id))->hooks();
        (new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\AddMethodReminder\TrackerData($this->method_id, $this->tracker_name))->hooks();
    }
    public function display_notice_if_should()
    {
        $display_strategy = new \FlexibleShippingDhlExpressProVendor\WPDesk\ShowDecision\GetStrategy([['page' => 'wc-settings', 'tab' => 'shipping', 'section' => $this->method_id]]);
        if ($display_strategy->shouldDisplay() && $this->shipping_method_settings_saved() && !$this->method_added_in_any_zone($this->method_id)) {
            $notice = new \FlexibleShippingDhlExpressProVendor\WPDesk\Notice\Notice(\sprintf(\__('%1$sPlease add now the %2$s shipping method within the shipping zone you want it to be available in →%3$s', 'flexible-shipping-dhl-express-pro'), '<a href="' . \admin_url('admin.php?page=wc-settings&tab=shipping&track_click_notice_method=' . \wp_create_nonce($this->method_id)) . '">', $this->method_title, '</a>'), \FlexibleShippingDhlExpressProVendor\WPDesk\Notice\Notice::NOTICE_TYPE_WARNING);
        }
    }
