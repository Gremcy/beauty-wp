...
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShippingFields;
...
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\MethodFieldsFactory;
use FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod;
/**
...
trait SettingsTrait
{
     ...
     */
    protected function get_settings_definition_from_service(\FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
        ...
        if ($shipping_service instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanTestSettings) {
            $settings_definitions = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ApiStatus\ApiStatusSettingsDefinitionDecorator($settings_definitions, $shipping_service->get_field_before_api_status_field(), $plugin_shipping_decisions->get_field_api_status_ajax(), $shipping_service->get_unique_id());
        }
     ...
     * @return array
     */
    private function get_form_fields_from_shipping_service(\FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\PluginShippingDecisions $plugin_shipping_decisions)
    {
    ...
    public function generate_settings_html($form_fields = array(), $echo = \true)
    {
        ...
        foreach ($form_fields as $field_id => $values) {
            ...
            if ($options_generator === \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields::OPTIONS_GENERATOR_COUNTRY_STATE) {
                $values['options'] = $this->prepare_country_state_options();
            }
            if ($options_generator === \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields::OPTIONS_GENERATOR_COUNTRY_STATE_FOR_ORIGIN) {
    ...
    private function prepare_custom_field_types($fields)
    {
        ...
        if ($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $fields = $this->replace_handling_fees_field_if_exists($fields);
        }
        if ($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCustomOrigin) {
            $custom_origin_fields = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\CustomOriginFields($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin);
            $fields = $custom_origin_fields->replace_fallback_field_if_exists($fields, $this);
        }
        if ($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasInstanceCustomOrigin) {
            $instance_custom_origin_fields = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomOrigin\InstanceCustomOriginFields(\true);
            $fields = $instance_custom_origin_fields->replace_fallback_field_if_exists($fields, $this);
        }
        if ($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasFreeShipping) {
            $free_shipping_fields = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\FreeShipping\FreeShippingFields();
    ...
    protected function create_fields_factory()
    {
        if ($this->fields_factory === null) {
            $this->fields_factory = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\MethodFieldsFactory();
        }
    ...
    private function replace_fallback_field_if_exists($settings)
    {
        ...
        foreach ($settings as $key => $field) {
            if ($field['type'] === \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_TYPE_FALLBACK) {
                $new_settings[\FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_ENABLE_FALLBACK] = ['title' => \__('Fallback', 'flexible-shipping-dhl-express-pro'), 'type' => 'checkbox', 'label' => \__('Enable fallback', 'flexible-shipping-dhl-express-pro'), 'description' => \__('Enable to offer flat rate cost for shipping so that the user can still checkout, if API for some reason returns no matching rates.', 'flexible-shipping-dhl-express-pro'), 'desc_tip' => \true, 'default' => 'no'];
                $new_settings[\FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod::FIELD_FALLBACK_COST] = ['title' => \__('Fallback Cost', 'flexible-shipping-dhl-express-pro'), 'type' => 'price', 'required' => \true, 'description' => \__('Enter only a numeric value without the currency symbol.', 'flexible-shipping-dhl-express-pro'), 'desc_tip' => \true, 'default' => ''];
            } else {
    ...
    private function replace_handling_fees_field_if_exists($settings)
    {
        $new_settings = [];
        foreach ($settings as $key => $field) {
            if ($field['type'] === \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::FIELD_TYPE) {
                $field_handling_fees = new \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees();
    ...
    {
        foreach ($settings as $key => $field) {
            if (isset($field['type']) && \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::FIELD_TYPE === $field['type']) {
                $settings[$key]['sanitize_callback'] = [\FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\Services\FieldServices::class, 'sanitize'];
            }
        }
        return $settings;
    }
    ...
    public function admin_options()
    {
        ...
        /** @TODO: move to custom field & field footer. */
        if ($this instanceof \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasHandlingFees) {
            $price_adjustment_type_field = $settings_prefix . '_' . \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_TYPE;
            $price_adjustment_value_field = $settings_prefix . '_' . \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\CustomFields\FieldHandlingFees::OPTION_PRICE_ADJUSTMENT_VALUE;
            $price_adjustment_type_none = \FlexibleShippingDhlExpressProVendor\WPDesk\WooCommerceShipping\HandlingFees\PriceAdjustmentNone::ADJUSTMENT_TYPE;
    ...
    {
        return new \FlexibleShippingDhlExpressProVendor\WPDesk\AbstractShipping\Settings\SettingsValuesAsArray(\array_merge($this->settings, $this->instance_id ? $this->instance_settings : []));
    }
}
