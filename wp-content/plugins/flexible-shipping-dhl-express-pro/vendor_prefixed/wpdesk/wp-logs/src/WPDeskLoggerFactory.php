...
use LogicException;
use FlexibleShippingDhlExpressProVendor\Monolog\Handler\NullHandler;
use FlexibleShippingDhlExpressProVendor\Monolog\Logger;
use FlexibleShippingDhlExpressProVendor\Monolog\Registry;
use FlexibleShippingDhlExpressProVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use FlexibleShippingDhlExpressProVendor\WPDesk\Logger\WC\WooCommerceCapture;
use FlexibleShippingDhlExpressProVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \FlexibleShippingDhlExpressProVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\FlexibleShippingDhlExpressProVendor\Monolog\Registry::hasLogger($name)) {
            \FlexibleShippingDhlExpressProVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\FlexibleShippingDhlExpressProVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \FlexibleShippingDhlExpressProVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \FlexibleShippingDhlExpressProVendor\Monolog\Logger($name);
        }
        if (\FlexibleShippingDhlExpressProVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \FlexibleShippingDhlExpressProVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\FlexibleShippingDhlExpressProVendor\Monolog\Logger $logger)
    {
        if (!\defined('FlexibleShippingDhlExpressProVendor\\WC_LOG_THRESHOLD')) {
            \define('FlexibleShippingDhlExpressProVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \FlexibleShippingDhlExpressProVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \FlexibleShippingDhlExpressProVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \FlexibleShippingDhlExpressProVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \FlexibleShippingDhlExpressProVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\FlexibleShippingDhlExpressProVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \FlexibleShippingDhlExpressProVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \FlexibleShippingDhlExpressProVendor\Monolog\Registry::hasLogger($name);
    }
}
