...
use FlexibleShippingDhlExpressProVendor\WPDesk\Notice\AjaxHandler;
use FlexibleShippingDhlExpressProVendor\WPDesk\Notice\PermanentDismissibleNotice;
if (!\defined('ABSPATH')) {
    exit;
}
if (!\class_exists('FlexibleShippingDhlExpressProVendor\\WPDesk_API_Manager_With_Update_Flag')) {
    class WPDesk_API_Manager_With_Update_Flag
    {
         ...
         */
        public $text_domain = 'flexible-shipping-dhl-express-pro';
        /**
         * @var string
         */
        public $plugin_url;
        ...
        {
            $password_management = new \FlexibleShippingDhlExpressProVendor\WPDesk_API_Password_Management();
        ...
        public function key()
        {
            if ($this->key_insance == null) {
                $this->key_insance = new \FlexibleShippingDhlExpressProVendor\WPDesk_API_KEY($this);
            }
        ...
        public function update_check($upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '')
        {
            if ($this->update_check_insance == null) {
                $this->update_check_insance = new \FlexibleShippingDhlExpressProVendor\WPDesk_Update_API_Check($this, $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra);
            }
        ...
        public function check_external_blocking()
        {
            ...
            if (\defined('FlexibleShippingDhlExpressProVendor\\WP_HTTP_BLOCK_EXTERNAL') && \FlexibleShippingDhlExpressProVendor\WP_HTTP_BLOCK_EXTERNAL === \true) {
                // check if our API endpoint is in the allowed hosts
                $host = \parse_url($this->upgrade_url, \PHP_URL_HOST);
                if (!\defined('FlexibleShippingDhlExpressProVendor\\WP_ACCESSIBLE_HOSTS') || \stristr(\FlexibleShippingDhlExpressProVendor\WP_ACCESSIBLE_HOSTS, $host) === \false) {
                    ?>
                    <div class="error">
                        <p><?php 
        ...
        {
            $parser = new \FlexibleShippingDhlExpressProVendor\WPDesk\License\Changelog\Parser($changelog);
            $parser->parse();
            $parsed_changelog = $parser->get_parsed_changelog()->getIterator();
            $changes = new \FlexibleShippingDhlExpressProVendor\WPDesk\License\Changelog\Filter\ByVersion($parsed_changelog, $plugin_version);
            if (\iterator_count($changes) > 0) {
                $changelog = new \FlexibleShippingDhlExpressProVendor\WPDesk\License\Changelog\Formatter($changes);
