...
namespace FlexibleShippingDhlExpressProVendor\WPDesk\License\Page\License\Action;

use FlexibleShippingDhlExpressProVendor\WPDesk\License\Page\Action;
use FlexibleShippingDhlExpressProVendor\WPDesk\License\ServerAddressRepository;
 ...
 */
class LicenseActivation extends \FlexibleShippingDhlExpressProVendor\WPDesk\License\Page\License\Action\AbstractAction implements \FlexibleShippingDhlExpressProVendor\WPDesk\License\Page\Action
{
    /**
     * Plugin data.
     *
    ...
    public function execute(array $plugin)
    {
        ...
        $this->activate_license($activation_email, $api_key, new \FlexibleShippingDhlExpressProVendor\WPDesk\License\ServerAddressRepository($product_id));
    }
    /**
    ...
    {
        $this->add_error('api_key_check_text', 'api_key_check_error', \__('Connection failed to the Subscription Key API server. Try again later.', 'flexible-shipping-dhl-express-pro'), 'error');
    }
    /**
    ...
    {
        $this->add_error('activate_text', 'activate_msg', \__('Plugin license activated. ', 'flexible-shipping-dhl-express-pro') . "{$activate_results['message']}.", 'updated');
    }
    /**
     * Activate license.
     *
     ...
     */
    public function activate_license($activation_email, $api_key, \FlexibleShippingDhlExpressProVendor\WPDesk\License\ServerAddressRepository $address_repository)
    {
