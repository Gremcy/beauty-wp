...
use FlexibleShippingDhlExpressProVendor\WPDesk\License\InstalledPlugins;
/**
 * Can render and manage license page.
 *
 * @package WPDesk\License\Page\License
 */
class LicensePage implements \FlexibleShippingDhlExpressProVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function __construct(\FlexibleShippingDhlExpressProVendor\WPDesk\License\InstalledPlugins $plugin_database)
    {
    ...
    {
        \add_submenu_page('wpdesk-helper', \__('Subscriptions', 'flexible-shipping-dhl-express-pro'), \__('Subscriptions', 'flexible-shipping-dhl-express-pro'), 'manage_options', self::PAGE_SLUG, [$this, 'handle_render_wpdesk_licenses_page']);
    }
    /**
    ...
    public function handle_render_wpdesk_licenses_page()
    {
        ...
        if (!\class_exists('FlexibleShippingDhlExpressProVendor\\WPDesk_Helper_List_Table')) {
    ...
    private function execute_plugin_action($plugin, $action)
    {
        ...
        if (isset($plugin_info)) {
            (new \FlexibleShippingDhlExpressProVendor\WPDesk\License\Page\LicensePageActions())->create_action($action)->execute($plugin_info);
        }
    }
    /**
