...
use FlexibleShippingDhlExpressProVendor\WPDesk\DhlExpressShippingService\Exception\DhlApiException;
class GetQuoteResponse
{
    ...
    public function initFromXML($xml)
    {
        ...
        if (isset($xml->Response, $xml->Response->Status, $xml->Response->Status->Condition, $xml->Response->Status->Condition->ConditionCode) && (string) $xml->Response->Status->Condition->ConditionCode != '') {
            throw new \FlexibleShippingDhlExpressProVendor\WPDesk\DhlExpressShippingService\Exception\DhlApiException(\esc_html($xml->Response->Status->Condition->ConditionData), (int) $xml->Response->Status->Condition->ConditionCode);
        }
        if (isset($xml->GetQuoteResponse, $xml->GetQuoteResponse->Note, $xml->GetQuoteResponse->Note->Condition)) {
            foreach ($xml->GetQuoteResponse->Note->Condition as $condition) {
                throw new \FlexibleShippingDhlExpressProVendor\WPDesk\DhlExpressShippingService\Exception\DhlApiException(\esc_html($condition->ConditionData), (int) $condition->ConditionCode);
            }
        }
