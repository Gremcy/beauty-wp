 ...
 */
class DhlRateReplyInterpretation implements \FlexibleShippingDhlExpressProVendor\WPDesk\AbstractShipping\Rate\ShipmentRating
{
    /**
     * Is tax enabled.
     *
     ...
     */
    public function __construct(\FlexibleShippingDhlExpressProVendor\DHL\Entity\AM\GetQuoteResponse $reply, $is_tax_enabled, $shop_default_currency)
    {
        $this->reply = $reply;
    ...
    {
        $rate = new \FlexibleShippingDhlExpressProVendor\WPDesk\AbstractShipping\Rate\SingleRate();
        $rate->service_type = (string) $single_quote->GlobalProductCode;
        $rate->service_name = (string) $single_quote->ProductShortName;
        $money = new \FlexibleShippingDhlExpressProVendor\WPDesk\AbstractShipping\Rate\Money();
