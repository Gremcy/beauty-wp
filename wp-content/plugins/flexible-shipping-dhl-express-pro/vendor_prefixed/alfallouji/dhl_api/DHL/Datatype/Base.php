 ...
 * @version     0.1
 */
namespace FlexibleShippingDhlExpressProVendor\DHL\Datatype;
...
abstract class Base
{
    ...
    public function initFromXML($xml)
    {
        ...
        foreach ($xml->children() as $child) {
            ...
            if (isset($this->{$childName}) && \is_object($this->{$childName})) {
                ...
                $this->{$childName} = \trim((string) $child);
            }
        }
    }
    /**
    ...
    public function __call($name, $arguments)
    {
        ...
        if (isset($this->_params[$key . 's']) && $this->_params[$key . 's']['type'] != 'string' && isset($this->_params[$key . 's']['multivalues']) && \true === $this->_params[$key . 's']['multivalues']) {
            $key .= 's';
        }
    ...
    protected function validateParameters()
    {
        foreach ($this->_params as $name => $infos) {
            if ($this->_values[$name]) {
                if (\is_array($this->_values[$name]) && isset($infos['subobject']) && \true === $infos['subobject']) {
    ...
    protected function validateParameterType($key, $value)
    {
        ...
        switch ($this->_params[$key]['type']) {
            case 'string':
                if (\is_array($value) && isset($this->_params[$key]['multivalues']) && \true === $this->_params[$key]['multivalues']) {
                    foreach ($value as $subvalue) {
                        if (null !== $subvalue && $subvalue !== (string) $subvalue) {
                            throw new \InvalidArgumentException('Invalid type for ' . $key . '. It should be of type : ' . $this->_params[$key]['type'] . ' but it has a value of : ' . $subvalue);
                        }
                    }
            ...
            default:
                if (isset($this->_params[$key]['subobject']) && $this->_params[$key]['subobject']) {
                    ...
                    if (!$value instanceof $className) {
                        throw new \InvalidArgumentException('Invalid type for ' . $key . '. It should be of type : ' . $this->_params[$key]['type'] . ' but it has a value of : "' . \print_r($value, \true) . '"');
                    }
                }
                break;
        }
